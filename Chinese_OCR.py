...
# top 1 accuracy 0.8429017344486571 top k accuracy 0.9826597718346206
...
tf.app.flags.DEFINE_integer('charset_size', 12882, "Choose the first `charset_size` characters only.")
...
class DataIterator:
    def __init__(self, data_dir):
        ...
        print(truncate_path)
        # 遍歷訓練集所有圖像路徑，儲存在image_names内
        self.image_names = []
        for root, sub_folder, file_list in os.walk(data_dir):
            if root < truncate_path:
                self.image_names += [os.path.join(root, file_path) for file_path in file_list]
        random.shuffle(self.image_names) # 打亂
    ...
    def input_pipeline(self, batch_size, num_epochs=None, aug=False):
        ...
        # 將image_list ,label_list做一个slice處理
...
def build_graph(top_k):
    keep_prob = tf.placeholder(dtype=tf.float32, shape=[], name='keep_prob') # dropout打開機率
    ...
    with tf.device('/gpu:0'):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            logits = slim.fully_connected(slim.dropout(fc1, keep_prob), FLAGS.charset_size, activation_fn=None,
                                          scope='fc2')
        # 因为没有做熱編碼，所以使用sparse_softmax_cross_entropy_with_logits
...
def inference(name_list):
    ...
    # allow_soft_placement 如果你指定的裝置不存在，允許TF自動分配裝置
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options,allow_soft_placement=True)) as sess:
